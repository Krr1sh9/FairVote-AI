...
Historical constrained evidence folders were removed from `experiments/outputs/` to avoid presenting obsolete one-trial evidence as final submission evidence. The current canonical evidence path is:

```bash
python -m experiments.mrp_vs_baselines --preset final_evidence
```

The current run outputs are documented in `EXAMINER_RUN_GUIDE.md`, `docs/reproducing_experiments.md`, `docs/experiment_protocol.md`, and `docs/evidence_interpretation.md`.
...
- Final evidence must be generated by the examiner/student on available hardware; the repository no longer ships obsolete constrained result folders as if they were final evidence.
